...
(function(){
  ...
  +'<button class="bt" style=\"background:#28a745;color:#fff;border:none\" onclick="G()">Print</button>'
  +'<button class="bt" style=\"background:#dc3545;color:#fff;border:none\" onclick="CANCELP()">Cancel</button>'
  +'</div>'
  ...
  +'function US(){var tot=0,ok=0,al=0;document.querySelectorAll("#tb tr").forEach(function(r){if(r.style.display==="none")return;tot++;if(r.classList.contains("w"))al++;else ok++});document.getElementById("sT").textContent=tot;document.getElementById("sO").textContent=ok;document.getElementById("sA").textContent=al}'
    +'function SFV(f,el){document.querySelectorAll(".si").forEach(function(s){s.classList.remove("on")});el.classList.add("on");document.querySelectorAll("#tb tr").forEach(function(r){var ci=r.querySelector(".rc");if(!ci)return;var i=parseInt(ci.dataset.i);if(isNaN(i))return;var d=DATA[i];var show=false;if(f==="cotg")show=d.vs==="Arrived at VRL";else if(f==="fg")show=d.vs==="Finished Goods"||d.vs==="Deliverable/PDI Complete";else if(f==="del")show=!!d.delivered;r.style.display=show?"":"none"});TR()}'
  +'function UV(){var cotg=0,fg=0,del=0;DATA.forEach(function(d){if(d.otg&&!d.delivered)cotg++;if(d.vs==="Finished Goods")fg++;if(d.delivered)del++});document.getElementById("sCotg").textContent=cotg;document.getElementById("sFg").textContent=fg;document.getElementById("sDel").textContent=del}'
  ...
  +'function STAB(idx,btn){document.querySelectorAll(".tab").forEach(function(t){t.classList.remove("on")});btn.classList.add("on");document.getElementById("mainView").style.display=idx===0?"":"none";document.getElementById("csatView").style.display=idx===1?"":"none";document.getElementById("arrView").style.display=idx===2?"":"none";if(idx===1&&!document.getElementById("csatView").innerHTML.trim()){fetch("http://localhost:3000/api/tab/csat").then(function(r){return r.text()}).then(function(h){document.getElementById("csatView").innerHTML=h;LOADCSAT()}).catch(function(e){document.getElementById("csatView").innerHTML="<div style=padding:60px;text-align:center;color:#c00>Error loading CSAT: "+e.message+"</div>"})}if(idx===2&&!document.getElementById("arrView").innerHTML.trim()){fetch("http://localhost:3000/api/tab/arrivals").then(function(r){return r.text()}).then(function(h){document.getElementById("arrView").innerHTML=h;LOADARR()}).catch(function(e){document.getElementById("arrView").innerHTML="<div style=padding:60px;text-align:center;color:#c00>Error loading Arrivals: "+e.message+"</div>"})}}'
