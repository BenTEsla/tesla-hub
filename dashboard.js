...
(function(){
  ...
  +'function P1(i){var d=DATA[i];if(!d)return;var ti=d.tims?[d.rn]:[];var pv=(d.b2b||d.pay==="THIRD_PARTY_LEASING")?[d.rn]:[];if(!ti.length&&!pv.length){alert("No DocGen docs needed for "+d.name);return}event.target.textContent="...";fetch("http://localhost:3000/api/print/docgen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tiRNs:ti,pvlRNs:pv})}).then(function(r){return r.json()}).then(function(j){event.target.textContent=j.downloaded+" docs";event.target.style.color="#28a745"}).catch(function(){event.target.textContent="Error";event.target.style.color="#c00"})}'
