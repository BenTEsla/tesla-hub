...
(function(){
  ...
  +'function CHKCEE(){var rns=DATA.filter(function(d){return!d.b2b}).map(function(d){return d.rn});if(!rns.length)return;fetch("http://localhost:3000/api/incentive/check",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({rns:rns})}).then(function(r){return r.json()}).then(function(j){Object.keys(j.results).forEach(function(rn){var el=document.getElementById("cee_"+rn);if(el&&j.results[rn]){el.innerHTML="<a href=\\"https://tesla.cee.trustia.ai/admin/folder/folder/?q="+rn+"\\" target=_blank style=\\"font-size:10px;background:#e8f5e9;color:#2e7d32;padding:1px 6px;border-radius:10px;text-decoration:none;font-weight:600\\">CEE</a>"}})}).catch(function(){})}'
  +'L();setTimeout(CHKCEE,3000);'
