...
(function(){
  ...
  +'function P1(i,btn){var d=DATA[i];if(!d)return;var ti=d.tims?[d.rn]:[];var pv=(d.b2b||d.pay==="THIRD_PARTY_LEASING")?[d.rn]:[];if(!ti.length&&!pv.length){btn.textContent="N/A";return}btn.textContent="...";fetch("http://localhost:3000/api/print/docgen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tiRNs:ti,pvlRNs:pv})}).then(function(r){return r.json()}).then(function(j){btn.textContent=j.downloaded+" docs";btn.style.color="#28a745"}).catch(function(){btn.textContent="Error";btn.style.color="#c00"})}'
