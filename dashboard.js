...
(function(){
  ...
  +'function P1(i){var d=DATA[i];if(!d)return;var m=document.createElement("div");m.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.5);z-index:100;display:flex;align-items:center;justify-content:center";var b=document.createElement("div");b.style.cssText="background:#fff;border-radius:8px;padding:24px;min-width:280px;text-align:center";b.textContent="Printing "+d.name+"...";m.appendChild(b);document.body.appendChild(m);var ti=d.tims?[d.rn]:[];var pv=(d.b2b||d.pay==="THIRD_PARTY_LEASING")?[d.rn]:[];fetch("http://localhost:3000/api/print/docgen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tiRNs:ti,pvlRNs:pv})}).then(function(r){return r.json()}).then(function(j){b.innerHTML=j.downloaded+" docs for "+d.name+"<br><button onclick=this.closest(String.fromCharCode(39)+"div"+String.fromCharCode(39)+").parentElement.remove() style=margin-top:12px;padding:8px 16px;background:#3e6ae1;color:#fff;border:none;border-radius:4px;cursor:pointer>OK</button>"}).catch(function(){b.textContent="Error";setTimeout(function(){m.remove()},2000)})}'
