...
(function(){
  ...
  +'<button class="bt" onclick="G()" style="background:#28a745;color:#fff;border:none;padding:8px 20px;border-radius:4px;font-size:13px;font-weight:600;cursor:pointer">Print</button><button class="bt" onclick="CANCELP()" style="background:#dc3545;color:#fff;border:none;padding:8px 16px;border-radius:4px;font-size:13px;font-weight:600;cursor:pointer;margin-left:6px">Cancel</button>'
  +'</div>'
  ...
  +'function CANCELP(){if(!confirm("Cancel all prints?"))return;fetch("http://localhost:3000/api/print/cancel",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}).then(function(r){return r.json()}).then(function(){alert("Print queue cleared!")}).catch(function(){alert("Error cancelling")})}'
  +'function STAB(idx,btn){document.querySelectorAll(".tab").forEach(function(t){t.classList.remove("on")});btn.classList.add("on");document.getElementById("mainView").style.display=idx===0?"":"none";document.getElementById("csatView").style.display=idx===1?"":"none";document.getElementById("arrView").style.display=idx===2?"":"none";}'
  ...
  +'L();setTimeout(LOADARR,2000);setTimeout(LOADCSAT,2500);'

  +'</scr'+'ipt>'
  +'</div>' // close mainView
  +'<div id="csatView" style="display:none">CSAT - Coming soon</div>'
  +'<div id="arrView" style="display:none">Arrivals - Coming soon</div>'
  +'</body></html>');
  w.document.close();
})();
