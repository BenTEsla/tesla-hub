// Tesla Delivery Hub v8 â€” blob iframe for perfect CSS isolation
(function(){
  ...
  b.onclick=function(){go()};document.body.appendChild(b);
})();
...
function go(){
  ...
  if(!AUTH){var _f=window.fetch;window.fetch=function(){var u=typeof arguments[0]==='string'?arguments[0]:'';var o=arguments[1]||{};var h=o.headers||{};if(u.indexOf('mytdeliveryopsapi')>=0&&(h.Authorization||h.authorization))AUTH={token:h.Authorization||h.authorization,userId:h.userid||h.UserId||ui||''};return _f.apply(this,arguments)};}
  ...
  +'tr.w td{background:#fffdf5}'
  +'tr.b td{background:#fef8f8}'
  ...
  +'<button class="btn bg" id="gn" style="display:none">Generer PDFs</button>'
  +'<button class="btn bl" id="cl">Fermer</button>'
  ...
  ifr.onload=function(){
    ...
    // Close
    $('cl').onclick=function(){document.getElementById('tdh-overlay').remove()};

    // Filters
    var fils=['all'].concat(CES);
    fils.forEach(function(f,i){
      var btn=$(i===0?'fa':'f'+(i-1));
      if(!btn)return;
    ...
    // Load
    $('ld').onclick=async function(){
      var lg=$('lg'),tbl=$('tbl'),tb=$('tb');
      lg.style.display='';tbl.style.display='none';
      lg.innerHTML='<span class="spin"></span> Chargement...';
      if(!AUTH){lg.innerHTML='Token non trouve. Rafraichis DRO (F5).';return}
      ...
      try{
        ...
        $('sT').textContent=items.length;$('sO').textContent=ok;$('sA').textContent=items.length-ok;
        lg.style.display='none';tbl.style.display='';$('gn').style.display='';
    ...
    $('gn').onclick=function(){
      ...
      D.querySelectorAll('.rc:checked').forEach(function(c){var tr=c.closest('tr');if(tr&&tr.style.display!=='none'){var i=parseInt(c.dataset.i);if(data[i])checked.push(data[i])}});
