...
(function(){
  ...
  // KEBAB MENU
  +'.kebab{background:none;border:1px solid rgba(255,255,255,.1);border-radius:8px;cursor:pointer;padding:6px 10px;color:#a1a1aa;font-size:18px;font-family:inherit;line-height:1;transition:all .15s;letter-spacing:2px}'
  +'.kebab:hover{background:rgba(255,255,255,.06);color:#f4f4f5}'
  +'.kmenu{position:absolute;right:0;top:100%;margin-top:8px;background:#1c1c22;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:6px;min-width:180px;box-shadow:0 8px 32px rgba(0,0,0,.4);z-index:100;display:none}'
  +'.kmenu.open{display:block}'
  +'.kmenu a,.kmenu button{display:flex;align-items:center;gap:8px;width:100%;padding:10px 14px;border:none;background:none;color:#d4d4d8;font-size:13px;font-family:inherit;font-weight:500;cursor:pointer;border-radius:6px;text-decoration:none;text-align:left;transition:background .1s}'
  +'.kmenu a:hover,.kmenu button:hover{background:rgba(255,255,255,.06);color:#f4f4f5}'
  ...
  // TITLE
  +'<div class="title-row"><h1 id="pageTitle" class="ttl">Customer Delivery</h1><div style="flex:1"></div><div id="tabActions" style="position:relative"></div><div id="themeAnchor" style="margin-left:12px"></div></div>'
  ...
  +'function NAV(idx,el){document.querySelectorAll(".nav-item").forEach(function(n){n.classList.remove("on")});el.classList.add("on");var titles=["Customer Delivery","Arrivals","Stock","Trade-In","CSAT"];var pt=document.getElementById("pageTitle");if(pt)pt.textContent=titles[idx]||"";var ta=document.getElementById("tabActions");if(ta){var menus=["","<button class=\\"kebab\\" onclick=\\"this.nextElementSibling.classList.toggle(\'open\')\\">\\u22EF</button><div class=\\"kmenu\\"><a href=\\"https://bi.teslamotors.com/#/views/EMEA-ETA2SC/VehicleArrival\\" target=\\"_blank\\">BI Export</a></div>","","<button class=\\"kebab\\" onclick=\\"this.nextElementSibling.classList.toggle(\'open\')\\">\\u22EF</button><div class=\\"kmenu\\"><button onclick=\\"ENRICHTI();this.closest(\'.kmenu\').classList.remove(\'open\')\\">Enrich DRO</button><button onclick=\\"TRIGGERSCAN(this);this.closest(\'.kmenu\').classList.remove(\'open\')\\">Scan</button><a href=\\"/scan.html\\" target=\\"_blank\\">Upload PDF</a></div>",""];ta.innerHTML=menus[idx]||""}STAB(idx,null)}'
  +'document.addEventListener("click",function(e){if(!e.target.closest(".kebab")&&!e.target.closest(".kmenu")){document.querySelectorAll(".kmenu.open").forEach(function(m){m.classList.remove("open")})}})'
