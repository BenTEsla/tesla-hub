...
(function(){
  ...
  +'<th style="width:75px">Payment<br><select class="fi" onchange="CF()" onclick="event.stopPropagation()"><option value="">All</option><option value="OK">OK</option><option value="No">No</option></select></th><th style="width:50px">Print</th>'
  ...
  +'<th style="width:75px">Insurance<br><select class="fi" onchange="CF()" onclick="event.stopPropagation()"><option value="">All</option><option value="OK">OK</option><option value="No">No</option></select></th><th style="width:50px">Print</th>'
  ...
  +'function RW(){var tb=document.getElementById("tb");var out="";for(var i=0;i<DATA.length;i++){var d=DATA[i];var vc=d.otg?"dg":(d.vs.indexOf("Transit")>=0?"do":"dr");var rc=d.regOk?"<span class=\\"dt dg\\"></span>OK":d.regTxt==="On Hold"?"<span class=\\"dt dr\\"></span><b style=color:#c00>On Hold</b>":d.regTxt==="RTS"?"<span class=\\"dt do\\"></span>RTS":d.regTxt==="Pending"?"<span class=\\"dt do\\"></span>Pending":"<span class=su>"+d.regTxt+"</span>";var tiOk=d.tims&&(d.tims.indexOf("Approved")>=0||d.tims.indexOf("Received")>=0);var tc=d.tims?(tiOk?"<span class=\\"dt dg\\"></span>"+d.tims:"<span class=\\"dt do\\"></span>"+d.tims):"<span class=su>No</span>";out+="<tr class=\\""+(d.al.length?"w":"")+"\\" data-host=\\""+d.host+"\\"><td><input type=checkbox class=\\"ck rc\\" data-i="+i+" "+(d.al.length===0?"checked":"")+"></td><td class=dtc>"+(d.sdd||"")+"</td><td>"+d.t+"</td><td><span class=nm>"+d.name+"</span></td><td><a class=rl href=\\"https://dro.tesla.com/advisor?sidepanel_fullscreen=yes&rn="+d.rn+"\\" target=_blank>"+d.rn+"</a></td><td>"+d.model+"</td><td>"+rc+"</td><td>"+(d.amtOk?"<span class=\\"dt dg\\"></span>OK":"<span class=\\"dt dr\\"></span>No")+"</td><td>"+tc+"</td><td><span class=\\"dt "+vc+"\\"></span>"+d.vs+"</td><td>"+(d.hold?"<span class=\\"dt dr\\"></span><b style=color:#c00>Hold</b>":"<span class=\\"dt dg\\"></span>OK")+"</td><td>"+(d.io?"<span class=\\"dt dg\\"></span>OK":"<span class=su>No</span>")+"</td><td><button onclick=\\"P1("+i+")\\" style=\\"padding:4px 8px;border:1px solid #ddd;border-radius:4px;cursor:pointer;background:#fff;font-size:12px\\">Print</button></td></tr>"}tb.innerHTML=out;if(WKMODE)showDateCol(true)}'

  +'function G(){var c=[];document.querySelectorAll(".rc:checked").forEach(function(el){var tr=el.closest("tr");if(tr&&tr.style.display!=="none"){var i=parseInt(el.dataset.i);if(DATA[i])c.push(DATA[i])}});if(!c.length){alert("No deliveries selected!");return}var ds=document.getElementById("dt").value;var FC={"Pearl White":"Blanc Perle","Diamond Black":"Noir Diamant","Stealth Grey":"Gris Stealth","Ultra Red":"Rouge Ultra","Quicksilver":"Quicksilver","Solid Black":"Noir"};var FP={"CASH":"CASH","TESLA_LEASING":"LEASING","TESLA_LENDING":"CR\\u00c9DIT","THIRD_PARTY_LEASING":"LLD TIERS"};var FPC={"CASH":"#28a745","TESLA_LEASING":"#1565c0","TESLA_LENDING":"#6a1b9a","THIRD_PARTY_LEASING":"#e65100"};var E=function(s){return String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;")};var html="";c.forEach(function(d){var tiBlock=d.tims?"<div class=bx style=\\"border-color:#c8e6c9;background:#f9fff9\\"><div class=bt>TRADE-IN</div><div style=\\"font-size:15px;font-weight:700\\">"+E(d.tims)+"</div></div>":"<div class=bx><div class=bt>TRADE-IN</div><div style=color:#ccc>None</div></div>";html+="<div class=pg><div style=\\"display:flex;justify-content:space-between\\"><div><div style=\\"font-size:28px;font-weight:700\\">"+E(d.name)+"</div><div style=\\"font-size:13px;color:#888\\">"+d.rn+"</div></div><div style=text-align:right><div style=\\"font-size:42px;font-weight:700;line-height:1\\">"+d.t+"</div><div style=\\"font-size:13px;color:#888\\">"+ds+"</div></div></div><div class=rw><div class=bx style=flex:1><div class=bt>V\\u00c9HICULE</div><div style=\\"font-size:16px;font-weight:700\\">"+E(d.model)+"</div><div class=dt>\\u00b7 "+E(d.color)+"</div>"+(d.batt?"<div style=\\"color:#28a745;font-weight:600\\">"+d.batt+"%</div>":"")+"</div><div class=bx style=flex:1><div class=bt>IDENTIFICATION</div><div class=lb>IMMATRICULATION</div><div style=\\"font-size:26px;font-weight:700;font-family:monospace\\">"+E(d.plate||"Pending")+"</div><div class=lb>VIN</div><div style=\\"font-size:14px;font-weight:600;font-family:monospace\\">"+E(d.vin||"")+"</div></div></div><div class=rw><div class=bx><div class=bt>PAIEMENT</div><div style=\\"font-size:24px;font-weight:700;color:"+(FPC[d.pay]||"#999")+"\\">"+E(FP[d.pay]||d.pay||"")+"</div></div>"+tiBlock+"</div><div class=bx style=border-style:dashed><div class=bt>NOTES</div><div style=height:80px></div></div><div style=\\"display:flex;justify-content:space-between;padding-top:8px;border-top:1px solid #e0e0e0;margin-top:12px\\"><span style=\\"font-size:11px;color:#888\\">"+E(d.host)+"</span><span style=\\"font-size:11px;letter-spacing:6px;color:#ccc\\">T E S L A</span></div></div>"});var w=window.open("","_blank");w.document.write("<!DOCTYPE html><html><head><meta charset=utf-8><title>Print</title><style>@page{size:A4 portrait;margin:12mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:sans-serif;color:#171a20;font-size:13px}.pg{page-break-after:always;padding:0;min-height:100vh}.rw{display:flex;gap:10px;margin-bottom:10px}.bx{border:1px solid #e0e0e0;border-radius:6px;padding:12px 14px;flex:1}.bt{font-size:10px;letter-spacing:2px;color:#888;margin-bottom:8px}.lb{font-size:9px;letter-spacing:2px;color:#aaa;margin-top:8px;margin-bottom:2px}.dt{font-size:12px;color:#393c41;margin:2px 0}@media print{.pg{page-break-after:always}}</style></head><body>"+html+"</body></html>");w.document.close();var tiR=c.filter(function(d){return d.tims}).map(function(d){return d.rn});var pvR=c.filter(function(d){return d.b2b||d.pay===\"THIRD_PARTY_LEASING\"}).map(function(d){return d.rn});if(tiR.length||pvR.length){var modal=document.createElement(\"div\");modal.style.cssText=\"position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.5);z-index:100;display:flex;align-items:center;justify-content:center\";var box=document.createElement(\"div\");box.style.cssText=\"background:#fff;border-radius:8px;padding:32px;min-width:300px;text-align:center\";box.innerHTML=\"<div style=\\\"font-size:14px\\\">Generating \"+tiR.length+\" Trade-In + \"+pvR.length+\" PVL documents...</div><div style=\\\"margin-top:12px;font-size:12px;color:#999\\\">This may take 30 seconds...</div>\";modal.appendChild(box);document.body.appendChild(modal);fetch(\"http://localhost:3000/api/print/docgen\",{method:\"POST\",headers:{\"Content-Type\":\"application/json\"},body:JSON.stringify({tiRNs:tiR,pvlRNs:pvR})}).then(function(r){return r.json()}).then(function(j){box.innerHTML=\"<div style=\\\"font-size:36px\\\">Done!</div><div style=\\\"font-size:16px;margin:8px 0\\\">\"+j.downloaded+\" documents downloaded</div><div style=\\\"margin-top:12px\\\"><button onclick=\\\"this.closest(\\x27div\\x27).parentElement.parentElement.remove()\\\" style=\\\"padding:10px 20px;background:#3e6ae1;color:#fff;border:none;border-radius:4px;cursor:pointer\\\">Close</button></div>\"}).catch(function(e){box.innerHTML=\"<div style=\\\"color:#c00\\\">Error: \"+e.message+\"</div>\";setTimeout(function(){modal.remove()},3000)})}else{alert(c.length+\" pages de garde ready!\")}}'
  ...
  +'function P1(i){var d=DATA[i];if(!d)return;var modal=document.createElement("div");modal.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.5);z-index:100;display:flex;align-items:center;justify-content:center";var box=document.createElement("div");box.style.cssText="background:#fff;border-radius:8px;padding:24px;min-width:280px;text-align:center";box.innerHTML="<div style=\"font-size:14px\">Printing "+d.name+"...</div>";modal.appendChild(box);document.body.appendChild(modal);var tiR=d.tims?[d.rn]:[];var pvR=(d.b2b||d.pay==="THIRD_PARTY_LEASING")?[d.rn]:[];fetch("http://localhost:3000/api/print/docgen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tiRNs:tiR,pvlRNs:pvR})}).then(function(r){return r.json()}).then(function(j){box.innerHTML="<div style=\"font-size:24px\">Done!</div><div style=\"margin:8px 0\">"+j.downloaded+" docs for "+d.name+"</div><button onclick=\"this.closest(\\x27div\\x27).parentElement.remove()\" style=\"padding:8px 16px;background:#3e6ae1;color:#fff;border:none;border-radius:4px;cursor:pointer\">Close</button>"}).catch(function(e){box.innerHTML="<div style=\"color:#c00\">Error</div>";setTimeout(function(){modal.remove()},2000)})}'
