...
(function(){
  ...
  +'function P1(i,btn){var d=DATA[i];if(!d)return;btn.textContent="...";var ds=document.getElementById("dt").value;window.open("http://localhost:3000/api/print/page-de-garde/"+d.rn+"?date="+ds);var ti=d.tims?[d.rn]:[];var pv=(d.b2b||d.pay==="THIRD_PARTY_LEASING")?[d.rn]:[];if(ti.length||pv.length){fetch("http://localhost:3000/api/print/docgen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tiRNs:ti,pvlRNs:pv})}).then(function(r){return r.json()}).then(function(j){btn.textContent=j.downloaded+" docs";btn.style.color="#28a745";btn.style.fontWeight="700";fetch("http://localhost:3000/api/print/status/"+d.rn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({docs:j.downloaded})});j.files.forEach(function(f){window.open("http://localhost:3000/api/print/download/"+f.fileName)})}).catch(function(){btn.textContent="Err";btn.style.color="#c00"})}else{btn.textContent="OK";btn.style.color="#28a745";btn.style.fontWeight="700";fetch("http://localhost:3000/api/print/status/"+d.rn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({docs:0})})}}'
