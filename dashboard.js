...
(function(){
  ...
  +'function P1(i,btn){var d=DATA[i];if(!d)return;btn.textContent="...";var ds=document.getElementById("dt").value;var pay={"CASH":"CASH","TESLA_LEASING":"LEASING","TESLA_LENDING":"CREDIT","THIRD_PARTY_LEASING":"LLD TIERS"}[d.pay]||d.pay||"";var w=window.open("","_blank");w.document.write("<html><head><style>@page{size:A4;margin:12mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:sans-serif;color:#171a20;font-size:13px}.bx{border:1px solid #e0e0e0;border-radius:6px;padding:12px 14px;flex:1}.bt{font-size:10px;letter-spacing:2px;color:#888;margin-bottom:8px}.lb{font-size:9px;letter-spacing:2px;color:#aaa;margin-top:8px;margin-bottom:2px}.rw{display:flex;gap:10px;margin-bottom:10px}</style></head><body><div style=display:flex;justify-content:space-between><div><div style=font-size:28px;font-weight:700>"+d.name+"</div><div style=font-size:13px;color:#888>"+d.rn+"</div></div><div style=text-align:right><div style=font-size:42px;font-weight:700;line-height:1>"+d.t+"</div><div style=font-size:13px;color:#888>"+ds+"</div></div></div><div class=rw style=margin-top:16px><div class=bx><div class=bt>VEHICULE</div><div style=font-size:16px;font-weight:700>"+d.model+"</div><div>"+d.color+"</div></div><div class=bx><div class=bt>IDENTIFICATION</div><div class=lb>IMMATRICULATION</div><div style=font-size:26px;font-weight:700;font-family:monospace>"+(d.plate||"Pending")+"</div><div class=lb>VIN</div><div style=font-size:14px;font-weight:600;font-family:monospace>"+(d.vin||"")+"</div></div></div><div class=rw><div class=bx><div class=bt>PAIEMENT</div><div style=font-size:24px;font-weight:700>"+pay+"</div></div><div class=bx style=border-style:dashed><div class=bt>NOTES</div><div style=height:80px></div></div></div><div style=display:flex;justify-content:space-between;padding-top:8px;border-top:1px solid #e0e0e0;margin-top:200px><span style=font-size:11px;color:#888>"+d.host+"</span><span style=font-size:11px;letter-spacing:6px;color:#ccc>T E S L A</span></div></body></html>");w.document.close();var ti=d.tims?[d.rn]:[];var pv=(d.b2b||d.pay==="THIRD_PARTY_LEASING")?[d.rn]:[];if(ti.length||pv.length){fetch("http://localhost:3000/api/print/docgen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tiRNs:ti,pvlRNs:pv})}).then(function(r){return r.json()}).then(function(j){btn.textContent=j.downloaded+" docs";btn.style.color="#28a745";j.files.forEach(function(f){window.open("http://localhost:3000/api/print/download/"+f.fileName)})}).catch(function(){btn.textContent="Err";btn.style.color="#c00"})}else{btn.textContent="OK";btn.style.color="#28a745"}}'
